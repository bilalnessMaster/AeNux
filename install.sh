...
handle_error() {
  zenity --error --title="Installation Failed" \
    --text="An error occurred during setup.\nCheck log at: $LOGFILE" \ --height=400 --width=400
  exit 1
}
...
if command -v wine &>/dev/null; then
  ...
  case $DISTRO in
    ...
    9) distro="archlinux"; origin="archlinux" ;;
    *) handle_error ;;
  esac
  if [[ "$origin" == "archlinux" ]]; then 

  echo "[*] Detected Arch Linux. Installing Wine via pacman..."
  # sudo pacman -Syu --noconfirm
  sudo pacman -S --noconfirm wine wine-gecko wine-mono winetricks cabextract

  else
   # also install the winetricks
  echo "[*] Detected Debian/Ubuntu. Installing Wine from WineHQ..."
  ...
  sudo apt install cabextract  purge winetricks-y
  fi
fi
